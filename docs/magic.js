...
var Accordion = function Accordion() {
  ...
  var wrapperProps = {
    "class": "Accordion ".concat(key).concat(cl ? " ".concat(cl) : '')
  };
  return div(wrapperProps, [title && h3(title), items.map(function (_ref2, id) {
    var title = _ref2.title,
        text = _ref2.text;
    var open = state.accordions[key];
    var itemProps = {
      "class": "AccordionItem".concat(open === id ? ' open' : '')
    };
    var headerProps = {
      onclick: [actions.accordion.toggle, {
        key: key,
        id: id
      }]
    };
    return div(itemProps, [title && h4(headerProps, title), text && div({
      "class": 'content'
    }, text)]);
  })]);
};
...
var BlogMonth = function BlogMonth(state) {
  ...
  return [h3(to ? Link({
    to: to
  }, title) : title), months.map(function (_ref5) {
...
var BlogYear = function BlogYear(state) {
  ...
  return div([h2(link ? Link({
    to: to
  }, year) : year), Object.entries(blog[year]).map(function (_ref7) {
...
var actions = {
  'accordion': {
    'toggle': function toggle(state, _ref20) {
      var key = _ref20.key,
          id = _ref20.id;
...
var pages = {
  '/accordion/': function accordion(state) {
    return [h1(state.title), p(['this is the ', Link({
      to: 'https://github.com/magic-modules'
    }, '@magic-modules'), ' Accordion component. ', state.description]), GitBadges('magic-modules/accordion'), h2({
      id: 'installation'
    }, 'installation:'), Pre('npm install @magic-modules/accordion'), h2({
      id: 'usage'
    }, 'usage:'), p('in a page or Component.'), Pre("\nexport const View = state => Accordion({\n  key: 'about',\n  items: [\n    { title: 'title', text: 'content' },\n    { title: 'title 2', text: 'content 2' },\n    {\n      title: 'title 3',\n      text: [\n        p('content'),\n        p('can also be an'),\n        p(['array', ' of', ' modules']),\n        Link({ to: 'https://magic.github.io' }, 'magic.github.io'),\n      ],\n    },\n  ],\n  title: 'optional title',\n  state,\n})\n"), h2({
      id: 'demo'
    }, 'demo'), p('the example above renders'), Accordion({
      key: 'about',
      items: [{
        title: 'title',
        text: 'content'
        ...
        title: 'title 3',
        text: [p('content'), p('can also be an'), p(['array', ' of', ' modules']), Link({
      ...
      id: 'css'
    }, 'css'), p(['by default, css is minimal. to adapt the styles for your theme,', ' add (and change) the following object to /assets/theme/{THEME_NAME}/index.mjs']), Pre("\n'.Accordion': {\n  h3: {}, // main title\n  '.AccordionItem': {\n    h4: {}, // item title\n\n    '.content': {}, // item content\n\n    '&.open': {\n      '.content': {}, // item content if opened\n    },\n  },\n}"), h2({
      id: 'source'
  ...
  '/accordion/404/': function accordion404() {
